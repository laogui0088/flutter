...
class ChatPage extends StatelessWidget {
  ...
  Widget? _buildItemViewForMessage(BuildContext context, Message message) {
    // 只为语音消息提供特殊处理，其他消息类型返回null使用默认处理
    if (message.isVoiceType) {
      return Container(
        ...
        child: Obx(() => ChatVoiceView(
          isISend: message.sendID == OpenIM.iMManager.userID,
          soundPath: message.soundElem?.soundPath,
          soundUrl: message.soundElem?.sourceUrl,
          duration: message.soundElem?.duration,
          isPlaying: logic.isPlaySound(message), // 响应式播放状态
        )),
      );
    }
    return null; // 其他消息类型使用默认处理
  }
  ...
  Widget _buildItemView(Message message) => ChatItemView(
        ...
        itemViewBuilder: _buildItemViewForMessage,
