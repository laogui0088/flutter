...
class _ChatItemViewState extends State<ChatItemView> {
  ...
  Widget _buildChildView() {
    ...
    if (_message.isTextType) {
      ...
      child = ChatVoiceView(
        isISend: _isISend,
        soundPath: _message.soundElem?.soundPath,
        soundUrl: _message.soundElem?.sourceUrl,
        duration: _message.soundElem?.duration,
        isPlaying: false, // 这里可以根据实际播放状态来设置
      );
      ...
      child = ChatLocationView(
        description: _message.locationElem?.description ?? '',
        latitude: _message.locationElem?.latitude ?? 0.0,
        longitude: _message.locationElem?.longitude ?? 0.0,
      );
    } else if (_message.isCardType) {
      child = ChatCarteView(
        cardElem: _message.cardElem!,
      );
    } else if (_message.isCustomFaceType) {
      child = ChatCustomEmojiView(
        isISend: _isISend,
        data: _message.faceElem?.data,
      );
